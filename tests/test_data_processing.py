...
class TestIsNumericColumn:
    ...
    def test_integer_types(self):
        ...
        assert is_numeric_column('int64') == True
        assert is_numeric_column('int32') == True
        assert is_numeric_column('int16') == True
        assert is_numeric_column('int8') == True
        assert is_numeric_column('uint64') == True
        assert is_numeric_column('uint32') == True
    
    def test_float_types(self):
        """Test detection of float data types."""
        assert is_numeric_column('float64') == True
        assert is_numeric_column('float32') == True
        assert is_numeric_column('float16') == True
    
    def test_non_numeric_types(self):
        """Test rejection of non-numeric data types."""
        assert is_numeric_column('object') == False
        assert is_numeric_column('string') == False
        assert is_numeric_column('bool') == False
        assert is_numeric_column('datetime64') == False
        assert is_numeric_column('category') == False
    ...
    def test_case_variations(self):
        ...
        assert is_numeric_column('INT64') == False
        assert is_numeric_column('OBJECT') == False
...
class TestDataTypeHandling:
    ...
    def test_sex_mapping_all_values(self):
        ...
        for sex_str, expected_numeric in Config.SEX_MAPPING.items():
